...
def test_complete_multi_session_workflow():
    ...
    print("📊 Initial State:")
    print(f"   Mode: {'Multi-session' if is_multi_session_mode(cast(Data, data)) else 'Legacy'}")
    ...
    for user_id, contestant_choice, amount, expected_session in test_bets:
        ...
        assert session_id == expected_session, f"Expected {expected_session}, got {session_id}"
    ...
    for user_id, balance in data["balances"].items():
        ...
        print(f"   {user_id}: {initial} → {balance} ({'+' if change >= 0 else ''}{change})")
    ...
    assert final_total == initial_total, f"Economic error: {final_total} != {initial_total}"
    print(f"   ✅ Economy balanced!")
    
    print(f"\n📈 Session Summary:")
    print(f"   Active Sessions: {len(data.get('active_sessions', []))}")
    print(f"   Closed Sessions: {len([s for s in data['betting_sessions'].values() if s.get('status') == 'closed'])}")
...
if __name__ == "__main__":
    ...
    print(f"   Contestant mapping cleared: {len(final_data.get('contestant_to_session', {})) == 0}")