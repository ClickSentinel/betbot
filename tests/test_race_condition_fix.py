...
class TestReactionBettingRaceConditionFix:
    ...
    def test_cleanup_old_programmatic_removals(self, betting_cog):
        ...
        betting_cog._programmatic_removals_timestamps[recent_key] = current_time - 10  # 10 seconds ago
        betting_cog._programmatic_removals_timestamps[old_key] = current_time - 60    # 60 seconds ago
    ...
    async def test_non_programmatic_removal_continues_processing(self, betting_cog):
        ...
        async def test_handler(payload_arg):
            ...
            if betting_cog._is_programmatic_removal(payload_arg.message_id, payload_arg.user_id, str(payload_arg.emoji)):
        ...
        try:
            ...
            assert programmatic_check_passed, "Handler should have passed the programmatic removal check"
        finally:
    ...
    def test_multiple_users_no_interference(self, betting_cog):
        ...
        for user in users:
            betting_cog._mark_programmatic_removal(message_id, user["id"], user["old_emoji"])
        
        # Verify each user's programmatic removal is tracked independently
        for user in users:
            assert betting_cog._is_programmatic_removal(message_id, user["id"], user["old_emoji"])
        ...
        for user in users:
            ...
            for other_user in other_users:
                ...
                betting_cog._mark_programmatic_removal(message_id, other_user["id"], other_user["old_emoji"])
