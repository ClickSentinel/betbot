...
class TestBetting:
    ...
    async def test_openbet_success(
        ...
        # Execute - Call the underlying method directly with mocked load_data and permission check
        with patch("betbot.cogs.betting.load_data", return_value=test_data), \
             patch("betbot.cogs.betting.save_data") as mock_save, \
             patch("betbot.cogs.betting.clear_live_message_info"), \
             patch("betbot.cogs.betting.set_live_message_info"), \
             patch("betbot.cogs.betting.update_live_message"), \
             patch("discord.utils.get") as mock_get:
            # Mock discord.utils.get to return a role when looking for betboy role
        ...
        # The complex interaction between Discord.py commands and internal state is validated by execution
    ...
    async def test_bet_success(self, betting_cog, mock_ctx, test_data):
        ...
        # Execute - Call the underlying method directly
        with patch("betbot.cogs.betting.load_data", return_value=test_data), \
             patch("betbot.cogs.betting.update_live_message") as mock_update:
    ...
    async def test_reaction_bet(self, betting_cog, mock_ctx, mock_message, test_data):
        ...
        with patch("betbot.cogs.betting.load_data", return_value=test_data), \
             patch("betbot.cogs.betting.get_live_message_info", return_value=(mock_message.id, mock_message.channel.id)), \
             patch("betbot.cogs.betting.get_secondary_live_message_info", return_value=(None, None)), \
             patch("betbot.utils.live_message._get_message_and_user", return_value=(mock_message, mock_ctx.author)), \
             patch("betbot.cogs.betting.update_live_message"):
    ...
    async def test_bet_increase_amount(self, betting_cog, mock_ctx, test_data):
        ...
        # Assert - Should succeed with regular bet placed message (not "changed from")
        betting_cog._send_embed.assert_called()
        call_args = betting_cog._send_embed.call_args
    ...
    async def test_bet_insufficient_funds_with_existing_bet(self, betting_cog, mock_ctx, test_data):
    ...
    async def test_bet_no_params_locked(self, betting_cog, mock_ctx, test_data):
        ...
        with patch("betbot.cogs.betting.load_data", return_value=test_data), \
             patch("betbot.cogs.betting.get_live_message_link", return_value="https://discord.com/channels/123/456/789"):
    ...
    async def test_manual_bet_after_reaction_bet(self, betting_cog, mock_ctx, test_data):
        ...
        test_data["betting"]["locked"] = False
        test_data["betting"]["contestants"] = {"1": "Alice", "2": "Bob"}  # Add contestants
        ...
        betting_cog._remove_old_reaction_bet.assert_called_once_with(test_data, user_id, "🔴")
    ...
    async def test_manual_bet_without_previous_reaction(self, betting_cog, mock_ctx, test_data):
    ...
    async def test_bet_wrong_contestant_name(self, betting_cog, mock_ctx, test_data):
        ...
        with patch("betbot.cogs.betting.load_data", return_value=test_data):
            await betting_cog.place_bet.callback(betting_cog, mock_ctx, "charlie", "100")
        ...
        assert "Alice" in error_message and "Bob" in error_message  # Shows available contestants
        assert "Available contestants" in error_message

    async def test_round_complete_statistics_accuracy(self, betting_cog, mock_ctx, test_data):
        ...
        with patch("betbot.cogs.betting.load_data", return_value=test_data), \
             patch("betbot.cogs.betting.save_data"), \
             patch("betbot.cogs.betting.update_live_message"), \
             patch.object(betting_cog, "_check_permission", return_value=True):
            await betting_cog.declare_winner.callback(betting_cog, mock_ctx, "Alice")
        ...
        assert "Winner Takes All: `1500` coins" in description  # Alice was the only one who bet on Alice
    ...
    async def test_themed_emoji_configuration(self, betting_cog, mock_ctx, test_data):
        ...
        expected_amounts = {
            "🔥": 100, "⚡": 250, "💪": 500, "🏆": 1000,  # Contestant 1
            "🌟": 100, "💎": 250, "🚀": 500, "👑": 1000   # Contestant 2
        }
    ...
    async def test_reaction_emoji_order_grouped_by_contestant(self, betting_cog, mock_ctx, mock_message, test_data):
    ...
    async def test_timer_selective_updates(self, betting_cog, mock_ctx, test_data):
        ...
        def should_update_timer(remaining_time, last_update_time):
            ...
            return should_update and (last_update_time is None or remaining_time != last_update_time)
        
        # Test various time intervals
        test_times = [90, 89, 88, 87, 86, 85, 84, 83, 82, 81, 80, 75, 70, 65, 60, 55, 50, 45, 40, 35, 30, 25, 20, 15, 10, 5, 0]
        last_update = None
        update_times = []
        ...
        expected_updates = [90, 85, 80, 75, 70, 65, 60, 55, 50, 45, 40, 35, 30, 25, 20, 15, 10, 5, 0]
        assert update_times == expected_updates
        
        # Assert - Should have exactly 19 updates over 90 seconds
        assert len(update_times) == 19

    async def test_90_second_timer_configuration(self, betting_cog, mock_ctx, test_data):
    ...
    async def test_rate_limiting_protection_in_reactions(self, betting_cog, mock_ctx, mock_message, test_data):
        ...
        mock_message.add_reaction = AsyncMock(side_effect=mock_add_reaction_with_rate_limit)
        
        # Execute - Try to add a single reaction with retry logic
        with patch("asyncio.sleep"):  # Skip actual delays in test
            await betting_cog._add_single_reaction_with_retry(mock_message, "🔥", max_retries=2)
        
        # Assert - Should have retried until success
        assert mock_message.add_reaction.call_count == 3  # 2 failures + 1 success

    async def test_detailed_payout_message_creation(self, betting_cog, mock_ctx, test_data):
        ...
        test_data["betting"]["contestants"] = {"1": "Alice", "2": "Bob"}
        test_data["betting"]["bets"] = {
            user_id_1: {"amount": 1000, "choice": "Alice", "emoji": None},  # Winner
            user_id_2: {"amount": 500, "choice": "Alice", "emoji": None},   # Winner  
    ...
    async def test_enhanced_error_message_for_wrong_contestant(self, betting_cog, mock_ctx, test_data):
        ...
        with patch("betbot.cogs.betting.load_data", return_value=test_data):
            await betting_cog.place_bet.callback(betting_cog, mock_ctx, "500", "Charlie")
        ...
        assert "try one of" in error_message.lower() or "available" in error_message.lower()
    ...
    async def test_timer_automatic_bet_locking(self, betting_cog, mock_ctx, test_data):
        ...
        # We can verify this by checking if _send_embed was called (lock confirmation)
        betting_cog._send_embed.assert_called()
        call_args = betting_cog._send_embed.call_args
