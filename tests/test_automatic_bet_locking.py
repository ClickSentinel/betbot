...
class TestAutomaticBetLocking:
    ...
    @pytest.mark.asyncio
    async def test_auto_lock_updates_live_message(self, mock_bot, mock_ctx, sample_betting_data):
        """Test that automatic bet locking updates the live message."""
        timer = BettingTimer(mock_bot)
        
        with patch('betbot.utils.betting_timer.load_data', return_value=sample_betting_data), \
             patch('betbot.utils.betting_timer.save_data') as mock_save, \
             patch('betbot.utils.betting_timer.update_live_message') as mock_update, \
             patch('betbot.utils.betting_timer.schedule_live_message_update') as mock_schedule:
    ...
    @pytest.mark.asyncio
    async def test_manual_lock_updates_live_message(self, mock_bot, mock_ctx, sample_betting_data):
        """Test that manual bet locking updates the live message."""
        betting_cog = Betting(mock_bot)
        
        with patch('betbot.cogs.betting.load_data', return_value=sample_betting_data), \
             patch('betbot.cogs.betting.save_data') as mock_save, \
             patch('betbot.cogs.betting.update_live_message') as mock_update, \
             patch('betbot.cogs.betting.schedule_live_message_update') as mock_schedule, \
             patch('betbot.cogs.betting.get_live_message_info', return_value=(None, None)), \
    ...
    @pytest.mark.asyncio
    async def test_last_moment_bet_during_auto_lock(self, mock_bot, mock_ctx, sample_betting_data):
    ...
    @pytest.mark.asyncio
    async def test_timer_update_sequence_with_batched_updates(self, mock_bot, mock_ctx, sample_betting_data):
        ...
        with patch('betbot.utils.betting_timer.load_data', return_value=sample_betting_data), \
            ...
            time_sequence = [start_time, start_time + 5.0, start_time + 10.0, start_time + 11.0]
    ...
    @pytest.mark.asyncio
    async def test_timer_respects_manual_lock_during_countdown(self, mock_bot, mock_ctx, sample_betting_data):
        ...
        with patch('betbot.utils.betting_timer.load_data', side_effect=[sample_betting_data, locked_data]), \
             patch('betbot.utils.betting_timer.save_data') as mock_save, \
             patch('time.time', side_effect=[1000.0, 1005.0]):  # 5 seconds into timer
    ...
    @pytest.mark.asyncio 
    async def test_bet_locking_order_of_operations(self, mock_bot, mock_ctx, sample_betting_data):
        ...
        with patch('betbot.cogs.betting.load_data', return_value=sample_betting_data), \
             patch('betbot.cogs.betting.save_data', side_effect=track_save_data), \
             patch('betbot.cogs.betting.update_live_message', side_effect=track_update_message), \
             patch('betbot.cogs.betting.schedule_live_message_update', side_effect=track_schedule_update), \
             patch('betbot.cogs.betting.get_live_message_info', return_value=(None, None)), \
             patch('betbot.cogs.betting.get_secondary_live_message_info', return_value=(None, None)):
            ...
            expected_order = ["save_data", "update_live_message", "schedule_live_message_update"]
            assert operation_order == expected_order, f"Expected {expected_order}, got {operation_order}"

    @pytest.mark.asyncio
    async def test_reaction_clearing_during_lock(self, mock_bot, mock_ctx, sample_betting_data):
        ...
        with patch('betbot.cogs.betting.load_data', return_value=sample_betting_data), \
             ...
             patch('betbot.cogs.betting.update_live_message') as mock_update, \
             patch('betbot.cogs.betting.schedule_live_message_update'), \
             patch('betbot.cogs.betting.get_live_message_info', return_value=(987654321, 111222333)), \
             patch('betbot.cogs.betting.get_secondary_live_message_info', return_value=(None, None)):
