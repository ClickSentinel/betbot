...
class TestReactionBetChanges:
    ...
    @pytest.mark.asyncio
    async def test_programmatic_removal_prevents_race_condition(
        self, betting_cog, test_data
    ):
        ...
        betting_cog._mark_programmatic_removal(
            mock_payload.message_id, mock_payload.user_id, str(mock_payload.emoji)
        )
        ...
        assert (
            user_id not in test_data["betting"]["bets"]
            or str(user_id) not in test_data["betting"]["bets"]
        )
