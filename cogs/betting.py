...
from typing import cast, Optional, Dict, List, Any, Tuple
import asyncio
import time

from config import (
    TOKEN, C1_EMOJIS, C2_EMOJIS, SEPARATOR_EMOJI, CONTESTANT_EMOJIS,
    COLOR_ERROR, COLOR_GOLD, COLOR_DARK_ORANGE, COLOR_INFO, COLOR_WARNING, COLOR_SUCCESS, COLOR_DARK_GRAY,
    BET_TIMER_DURATION, BET_TIMER_UPDATE_INTERVAL,
    # Centralized Messages
    MSG_BET_ALREADY_OPEN, MSG_BET_LOCKED, MSG_NO_ACTIVE_BET, MSG_BET_LOCKED_NO_NEW_BETS,
    MSG_NO_BETS_TO_CLOSE, MSG_INTERNAL_ERROR_LOCKED, MSG_AMOUNT_POSITIVE,
    MSG_INVALID_BET_FORMAT, MSG_NO_CONTESTANTS_DEFINED, MSG_PLACE_MANUAL_BET_INSTRUCTIONS,
    MSG_FAILED_SEND_LIVE_MESSAGE, MSG_BETTING_LOCKED_SUMMARY, MSG_BETTING_TIMER_EXPIRED_SUMMARY,
    MSG_LIVE_BET_INITIAL_DESCRIPTION, MSG_A_WINNER_DECLARED_SOON, MSG_WAIT_FOR_ADMIN_TO_START,
    # Centralized Titles
    TITLE_BETTING_ERROR, TITLE_CANNOT_LOCK_BETS, TITLE_BETS_LOCKED, TITLE_BETTING_ROUND_OPENED,
    TITLE_INVALID_BET_FORMAT, TITLE_INVALID_CHOICE, TITLE_INVALID_AMOUNT, TITLE_BET_PLACED,
    TITLE_CANNOT_CLOSE_BETS, TITLE_INTERNAL_ERROR, TITLE_BETTING_CHANNEL_SET, TITLE_TIMER_TOGGLED,
    TITLE_CURRENT_BETS_OVERVIEW, TITLE_BETTING_LOCKED_OVERVIEW, TITLE_NO_ACTIVE_BETTING_ROUND,
    TITLE_LIVE_BETTING_ROUND, TITLE_POT_LOST, MSG_NO_ACTIVE_BET_AND_MISSING_ARGS,
    TITLE_NO_OPEN_BETTING_ROUND, MSG_BET_CHANGED
)
from data_manager import load_data, save_data, ensure_user, Data
from utils.live_message import (
    get_live_message_info, get_saved_bet_channel_id, set_live_message_info,
    set_secondary_live_message_info, clear_live_message_info, update_live_message,
    _get_message_and_user, _get_contestant_from_emoji, _get_condensed_bet_summary,
    get_live_message_link, _generate_timer_display,
...
class Betting(commands.Cog):
    ...
    def _find_contestant_info(self, data: Data, choice_input: str) -> Optional[Tuple[str, str]]:
    ...
    async def _add_betting_reactions(self, message: discord.Message, data: Data) -> None:
        ...
        all_emojis_to_add = data["contestant_1_emojis"] + [SEPARATOR_EMOJI] + data["contestant_2_emojis"]
    ...
    async def _remove_user_betting_reactions(self, message: discord.Message, user: discord.abc.User, data: Data) -> None:
    ...
    async def _process_winner_declaration(self, ctx: commands.Context, data: Data, winner_name: str) -> None:
        ...
        if winner_id is None:
            await self._send_embed(ctx, TITLE_BETTING_ERROR, f"Contestant '{winner_name}' not found.", COLOR_ERROR)
            return

        total_pot = sum(bet["amount"] for bet in bets.values())
        winning_bets = {uid: bet for uid, bet in bets.items() if bet["choice"] == winner_name.lower()}
        ...
        if total_winning_bet_amount == 0:
            ...
            await self._send_embed(ctx, TITLE_POT_LOST,
            ...
            for user_id, bet_info in winning_bets.items():
                winnings = int((bet_info["amount"] / total_winning_bet_amount) * total_pot)
                data["balances"][user_id] += winnings
                winnings_info[user_id] = winnings

            await self._send_embed(ctx, TITLE_BETTING_ROUND_OPENED,
        ...
        await update_live_message(self.bot, data, winner_declared=True, winner_name=winner_name, winnings_info=winnings_info)
    ...
    async def _lock_bets_internal(self, ctx: commands.Context, timer_expired: bool = False) -> None:
        """Internal logic to lock bets, callable by command or timer."""
        data = load_data()
        if not data["betting"]["open"]:
            msg = "⚠️ Betting is **already locked**." if data["betting"]["locked"] else MSG_NO_ACTIVE_BET + " to lock."
            await self._send_embed(ctx, TITLE_CANNOT_LOCK_BETS, msg, COLOR_ERROR)
            return

        data["betting"]["open"] = False
        data["betting"]["locked"] = True
        self._clear_timer_state_in_data(data) # Clear timer_end_time when bets are locked
        save_data(data)
        ...
        for msg_id, chan_id in messages_to_clear_reactions:
            ...
            if channel and isinstance(channel, discord.TextChannel):
                try:
                    ...
                    print(f"Cleared all reactions from live message {msg_id} in channel {chan_id}.")
                except discord.NotFound:
                    print(f"Live message {msg_id} not found when trying to clear reactions.")
                except discord.HTTPException as e:
                    print(f"Error clearing reactions from live message {msg_id}: {e}")
        # End re-added functionality

        await update_live_message(self.bot, data, betting_closed=True, close_summary=lock_summary)
    ...
    async def openbet(self, ctx: commands.Context, name1: str, name2: str) -> None:
        data = load_data()
        if data["betting"]["open"]:
            await self._send_embed(ctx, TITLE_BETTING_ERROR, MSG_BET_ALREADY_OPEN, COLOR_ERROR)
            return
        if data["betting"]["locked"]:
            await self._send_embed(ctx, TITLE_BETTING_ERROR, MSG_BET_LOCKED, COLOR_ERROR)
            return
        ...
        if target_channel is None:
            await self._send_embed(ctx, TITLE_BETTING_ERROR, "Betting rounds can only be opened in a server text channel.", COLOR_ERROR)
            return

        initial_embed_description = MSG_LIVE_BET_INITIAL_DESCRIPTION.format(
            contestant1_emoji=CONTESTANT_EMOJIS[0], name1=name1,
            contestant2_emoji=CONTESTANT_EMOJIS[1], name2=name2
        )

        main_live_msg = None
        try:
            # target_channel is now guaranteed to be a discord.TextChannel
            main_live_msg = await target_channel.send(embed=discord.Embed(title=TITLE_LIVE_BETTING_ROUND, description=initial_embed_description, color=COLOR_GOLD))
            ...
            await update_live_message(self.bot, data) # Call update_live_message here to populate the embed
        except Exception as e:
            print(f"Error sending main live message: {e}")
            set_live_message_info(data, None, None)
            await self._send_embed(ctx, TITLE_BETTING_ERROR, MSG_FAILED_SEND_LIVE_MESSAGE, COLOR_ERROR)
            return
        ...
        if isinstance(ctx.channel, discord.TextChannel) and ctx.channel.id != target_channel.id:
            try:
                secondary_live_msg = await ctx.channel.send(embed=discord.Embed(title=TITLE_LIVE_BETTING_ROUND, description=initial_embed_description, color=COLOR_GOLD))
                set_secondary_live_message_info(data, secondary_live_msg.id, ctx.channel.id)
            except Exception as e:
        ...
        if isinstance(ctx.channel, discord.TextChannel) and ctx.channel.id != target_channel.id: # Only send the detailed message if not the main betting channel
            if data["settings"]["enable_bet_timer"]:
                await self._send_embed(ctx, TITLE_BETTING_ROUND_OPENED,
            ...
            await self._send_embed(ctx, "✅ Betting Round Opened",
        ...
        if data["settings"]["enable_bet_timer"]:
            self.bet_timer_task = self.bot.loop.create_task(self._run_bet_timer(ctx, BET_TIMER_DURATION))
    ...
    async def declare_winner(self, ctx: commands.Context, winner: str) -> None:
        ...
        if not data["betting"]["locked"]:
            await self._send_embed(ctx, TITLE_CANNOT_CLOSE_BETS, MSG_INTERNAL_ERROR_LOCKED, COLOR_ERROR)
            return
    ...
    async def close_bet(self, ctx: commands.Context, winner: str) -> None:
        ...
        if not data["betting"]["open"] and not data["betting"]["locked"]:
            await self._send_embed(ctx, TITLE_CANNOT_CLOSE_BETS, MSG_NO_BETS_TO_CLOSE, COLOR_ERROR)
            return

        if data["betting"]["open"]:
            await self._lock_bets_internal(ctx) # This will set data["betting"]["locked"] = True
    ...
    @commands.has_permissions(manage_guild=True)
    async def set_bet_channel(self, ctx: commands.Context, channel: Optional[discord.TextChannel] = None) -> None:
        data = load_data()
        if channel is None:
            if isinstance(ctx.channel, discord.TextChannel):
                channel = ctx.channel
            else:
                await self._send_embed(ctx, TITLE_BETTING_ERROR, "This command must be used in a server text channel or by specifying a channel.", COLOR_ERROR)
                return
        
        set_live_message_info(data, None, channel.id) # Clear existing message, set new channel
        save_data(data)
        await self._send_embed(ctx, TITLE_BETTING_CHANNEL_SET,
    ...
    async def toggle_bet_timer(self, ctx: commands.Context) -> None:
        ...
        await self._send_embed(ctx, TITLE_TIMER_TOGGLED,
    ...
    async def place_bet(self, ctx: commands.Context, *args) -> None:
        ...
        if not data["betting"]["open"]:
            await self._send_embed(ctx, TITLE_NO_OPEN_BETTING_ROUND, MSG_NO_ACTIVE_BET_AND_MISSING_ARGS, COLOR_ERROR)
            return

        if data["betting"]["locked"]:
            await self._send_embed(ctx, TITLE_BETTING_ERROR, MSG_BET_LOCKED_NO_NEW_BETS, COLOR_ERROR)
            return
        ...
        if len(args) == 2:
            try:
                ...
                try:
                    ...
                    await self._send_embed(ctx, TITLE_INVALID_BET_FORMAT, MSG_INVALID_BET_FORMAT, COLOR_ERROR)
                    return
        else:
        ...
        if amount <= 0:
            await self._send_embed(ctx, TITLE_BETTING_ERROR, MSG_AMOUNT_POSITIVE, COLOR_ERROR)
            return

        contestant_info = self._find_contestant_info(data, choice)
        if not contestant_info:
            await self._send_embed(ctx, TITLE_BETTING_ERROR, MSG_INVALID_BET_FORMAT, COLOR_ERROR)
            return
        ...
        if amount > user_balance:
            await self._send_embed(ctx, TITLE_BETTING_ERROR, f"Insufficient balance! Your current balance is `{user_balance}` coins.", COLOR_ERROR)
            return
        ...
        await self._send_embed(ctx, TITLE_BET_PLACED, f"Your bet of `{amount}` coins on **{contestant_name}** has been placed!", COLOR_SUCCESS)

    @place_bet.error
    async def place_bet_error(self, ctx: commands.Context, error: commands.CommandError):
        data = load_data()
        is_missing_or_bad_arg = isinstance(error, commands.MissingRequiredArgument) or isinstance(error, commands.BadArgument)

        if not data["betting"]["open"] and is_missing_or_bad_arg:
            await self._send_embed(ctx, TITLE_NO_OPEN_BETTING_ROUND, MSG_NO_ACTIVE_BET_AND_MISSING_ARGS, COLOR_ERROR)
            return
        elif not data["betting"]["open"]:
            await self._send_embed(ctx, TITLE_NO_OPEN_BETTING_ROUND, MSG_NO_ACTIVE_BET, COLOR_ERROR)
            return

        if is_missing_or_bad_arg:
            await self._send_embed(ctx, TITLE_INVALID_BET_FORMAT, MSG_INVALID_BET_FORMAT, COLOR_ERROR)
        else:
            await self._send_embed(ctx, TITLE_BETTING_ERROR, f"An unexpected error occurred: {error}", COLOR_ERROR)

    @commands.command(name="mybet")
    async def mybet(self, ctx: commands.Context) -> None:
        data = load_data()
        if not data["betting"]["open"]:
            await self._send_embed(ctx, TITLE_BETTING_ERROR, MSG_NO_ACTIVE_BET, COLOR_ERROR)
            return
        ...
        if not user_bet:
            await self._send_embed(ctx, TITLE_BETTING_ERROR, "You have not placed any bets in the current round.", COLOR_ERROR)
            return
        ...
        await self._send_embed(ctx, TITLE_CURRENT_BETS_OVERVIEW,
    ...
    async def bettinginfo(self, ctx: commands.Context) -> None:
        ...
        live_message_link = get_live_message_link(self.bot, data, data["betting"]["open"] or data["betting"]["locked"])
    ...
    async def set_bet_timer(self, ctx: commands.Context, seconds: int) -> None:
        data = load_data()
        if seconds < 0:
            await self._send_embed(ctx, TITLE_BETTING_ERROR, "Timer duration cannot be negative.", COLOR_ERROR)
            return

        data["settings"]["bet_timer_duration"] = seconds
        save_data(data)

        await self._send_embed(ctx, TITLE_TIMER_TOGGLED, f"Bet timer duration set to `{seconds}` seconds.", COLOR_SUCCESS)
    ...
    async def betting_summary(self, ctx: commands.Context) -> None:
        ...
        contestant1_bets = sum(1 for bet in data["betting"]["bets"].values() if bet["choice"] == "contestant_1")
        contestant2_bets = sum(1 for bet in data["betting"]["bets"].values() if bet["choice"] == "contestant_2")

        await self._send_embed(ctx, TITLE_BETTING_LOCKED_OVERVIEW,
    ...
    async def manual_bet(self, ctx: commands.Context, user: discord.User, amount: int, *, choice: str) -> None:
        data = load_data()
        if not data["betting"]["open"]:
            await self._send_embed(ctx, TITLE_BETTING_ERROR, MSG_NO_ACTIVE_BET, COLOR_ERROR)
            return
        ...
        if amount > user_balance:
            await self._send_embed(ctx, TITLE_BETTING_ERROR, f"Insufficient balance! User's current balance is `{user_balance}` coins.", COLOR_ERROR)
            return
        ...
        await self._send_embed(ctx, TITLE_BET_PLACED, f"Manual bet of `{amount}` coins on **{contestant_name}** has been placed for {user.mention}!", COLOR_SUCCESS)

    @commands.command(name="setbettargetchannel")
    @commands.has_permissions(manage_guild=True)
    async def set_bet_target_channel(self, ctx: commands.Context, channel: discord.TextChannel) -> None:
        data = load_data()
        data["settings"]["bet_channel_id"] = channel.id
        save_data(data)

        await self._send_embed(ctx, TITLE_BETTING_CHANNEL_SET, f"Betting target channel set to {channel.mention}.", COLOR_SUCCESS)
    ...
    async def on_raw_reaction_add(self, payload: discord.RawReactionActionEvent) -> None:
        ...
        is_main_message = (payload.message_id == main_msg_id and payload.channel_id == main_chan_id)
        is_secondary_message = (payload.message_id == secondary_msg_id and payload.channel_id == secondary_chan_id)
        ...
        if bet_amount > user_balance:
            ...
            embed = discord.Embed(title=TITLE_BETTING_ERROR, description=f"<@{user.id}>, insufficient balance! Your current balance is `{user_balance}` coins.", color=COLOR_ERROR)
        ...
        if previous_bet_info:
            ...
            previous_emoji = previous_bet_info.get("emoji") # Get the emoji from the stored bet
            ...
            if previous_emoji:
                try:
                    ...
                    print(f"Error removing previous reaction {previous_emoji} from user {user.name}: {e}")
            
            # Send bet changed message
            channel = self.bot.get_channel(payload.channel_id)
            if isinstance(channel, discord.TextChannel):
                embed = discord.Embed(title="🔄 Bet Changed", description=MSG_BET_CHANGED.format(
                    user_id=user.id, amount=bet_amount, old_contestant=old_contestant_name.capitalize(), new_contestant=contestant_name.capitalize()
                ), color=COLOR_INFO)
                await channel.send(embed=embed)
