...
class BettingUtils:
    ...
    @staticmethod
    def find_contestant_info(data: Data, choice_input: str) -> Optional[Tuple[str, str]]:
        ...
        for i, emoji in enumerate(all_emojis_to_add):
            try:
                ...
                if "rate limited" in str(e).lower() or "429" in str(e):
                    ...
                    logger.warning(f"Rate limited adding reaction {emoji}, waiting and retrying...")
                    await asyncio.sleep(1.0)
                    try:
                        await message.add_reaction(emoji)
                    except discord.HTTPException as retry_e:
                        logger.warning(f"Could not add reaction {emoji} after retry: {retry_e}")
                else:
                    logger.warning(f"Could not add reaction {emoji} to live message: {e}")
    ...
    async def remove_user_betting_reactions(
        ...
        for emoji_str in all_betting_emojis:
            ...
            try:
                ...
                logger.warning(f"Failed to remove reaction {emoji_str} from user {user.name}: {e}")