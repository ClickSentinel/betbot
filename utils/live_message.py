...
def _generate_bet_progress_bar(
    ...
    )  # Calculate blocks based on bar_length
...
def _get_condensed_bet_summary(data: Data, include_reaction_info: bool = False) -> str:
    ...
    contestant_bets: Dict[str, List[Dict[str, Any]]] = {
        c_id: [] for c_id in contestants
    }
    ...
    if include_reaction_info and data["betting"]["open"]:
        ...
        # Display C1 options
        reaction_options_text += (
            f"{CONTESTANT_EMOJIS[0]} {contestants.get('1', 'Contestant 1')}: "
        )
        reaction_options_text += " ".join(
            [f"{emoji} `{reaction_bet_amounts.get(emoji, 0)}`" for emoji in c1_emojis]
        )
        reaction_options_text += "\n"

        # Display C2 options
        reaction_options_text += (
            f"{CONTESTANT_EMOJIS[1]} {contestants.get('2', 'Contestant 2')}: "
        )
        reaction_options_text += " ".join(
            [f"{emoji} `{reaction_bet_amounts.get(emoji, 0)}`" for emoji in c2_emojis]
        )
        reaction_options_text += "\n"
...
async def _get_detailed_bet_list(
    bot: discord.Client, data: Data, winnings_info: Optional[Dict[str, int]] = None
) -> str:
...
async def update_live_message(
    ...
    if (
        data["settings"]["enable_bet_timer"]
        and open_bet
        and data.get("timer_end_time") is not None
    ):
        ...
        current_total_duration = (
            BET_TIMER_DURATION  # Assuming BET_TIMER_DURATION is the total duration
        )
    ...
    if winner_declared and winner_name:
        ...
        description_parts.append(
            await _get_detailed_bet_list(bot, data, winnings_info=winnings_info)
        )  # NEW: Add detailed bet list with winnings
        ...
        description_parts.append(
            await _get_detailed_bet_list(bot, data)
        )  # NEW: Add detailed bet list
    elif open_bet:
        if contestants:
            description_parts.append(
                f"**Contestants:**\n> {CONTESTANT_EMOJIS[0]} **{contestants.get('1', 'Contestant 1')}**\n> {CONTESTANT_EMOJIS[1]} **{contestants.get('2', 'Contestant 2')}**\n\n"
        ...
        if (
            current_remaining_time is not None
            and current_total_duration is not None
            and data["settings"]["enable_bet_timer"]
        ):
            description_parts.append(
                _generate_timer_display(current_remaining_time, current_total_duration)
            )
            description_parts.append("\n\n")

        description_parts.append(
            _get_condensed_bet_summary(data, include_reaction_info=True)
        )
