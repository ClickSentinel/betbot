...
async def test_robust_reaction_batching():
    ...
    async def mock_process_bet(channel, data, user_id, amount, choice, emoji, notify_user=True):
        ...
        if data["balances"][user_id_str] >= amount:
            ...
            bet_processed = True
            final_bet_info = {"choice": choice.lower(), "amount": amount, "emoji": emoji}
            return True
        return False
    ...
    with patch('cogs.betting.load_data', return_value=mock_data), \
        ...
        for emoji in reactions:
            ...
            cog._pending_reaction_bets[123] = {
                ...
                "user": mock_user,
                "data": mock_data,
                "contestant_name": "Bob" if emoji in ["🌟", "💎", "🚀", "👑"] else "Alice",
        ...
        assert final_bet_info["choice"] == "bob", f"Expected 'bob', got {final_bet_info['choice']}"
        assert final_bet_info["emoji"] == "🌟", f"Expected '🌟', got {final_bet_info['emoji']}"
        assert mock_data["balances"]["123"] == 900, f"Expected 900, got {mock_data['balances']['123']}"
        ...
        asyncio.create_task(cog._backup_reaction_processing(456, 1.0))  # Shorter delay for testing
        
        # Wait for backup to trigger
        await asyncio.sleep(1.5)
        
        assert bet_processed, "Backup processing should have processed the bet"
        assert final_bet_info["choice"] == "alice", f"Expected 'alice', got {final_bet_info['choice']}"
        assert final_bet_info["amount"] == 250, f"Expected 250, got {final_bet_info['amount']}"
        ...
        assert mock_data["balances"]["123"] == first_balance, "Backup should not have processed again"
