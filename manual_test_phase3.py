...
def simulate_opensession_command(data, session_id, contestant1, contestant2, timer_duration=300):
...
def simulate_sessioninfo_command(data, session_id):
    ...
    if bets:
        ...
        for contestant_name, stats in contestant_stats.items():
            ...
            print(f"   - {contestant_name}: {stats['bets']} bets, {stats['pot']} coins ({percentage:.1f}%)")


def simulate_closesession_command(data, session_id, winner=None):
    """Simulate the !closesession command logic."""
    print(f"\n🔒 Closing session '{session_id}'" + (f" with winner '{winner}'" if winner else ""))
    ...
    if winner:
        ...
        if not winner_found:
            ...
            print(f"❌ Winner '{winner}' not found. Valid contestants: {contestants_list}")
            return False
        ...
        if winning_bets:
            for user_id, bet in bets.items():
                ...
                if bet["choice"] == winner.lower():
                    ...
                    print(f"   - {user_id}: {initial_balance} → {data['balances'][user_id]} (+{winnings})")
...
def main():
    ...
    success = simulate_opensession_command(data, "nfl_patriots_cowboys", "New England Patriots", "Dallas Cowboys", 600)
    print(f"Result: {'SUCCESS' if success else 'FAILED'}")
    
    # Test 2: Create second session  
    print(f"\n{'=' * 20} TEST 2: Create Second Session {'=' * 20}")
    success = simulate_opensession_command(data, "nba_lakers_warriors", "Los Angeles Lakers", "Golden State Warriors", 900)
    print(f"Result: {'SUCCESS' if success else 'FAILED'}")
    
    # Test 3: Try to create session with duplicate contestant
    print(f"\n{'=' * 20} TEST 3: Duplicate Contestant Test {'=' * 20}")
    success = simulate_opensession_command(data, "nfl_duplicate", "New England Patriots", "Buffalo Bills", 300)
    ...
    data["betting_sessions"]["nfl_patriots_cowboys"]["bets"]["user1"] = {
        "amount": 300, "choice": "new england patriots", "emoji": None
    }
    data["betting_sessions"]["nfl_patriots_cowboys"]["bets"]["user2"] = {
        "amount": 200, "choice": "dallas cowboys", "emoji": None
    }
    data["betting_sessions"]["nba_lakers_warriors"]["bets"]["user3"] = {
        "amount": 500, "choice": "los angeles lakers", "emoji": None
    }
    # Update balances
    ...
    success = simulate_closesession_command(data, "nfl_patriots_cowboys", "New England Patriots")
    print(f"Result: {'SUCCESS' if success else 'FAILED'}")
