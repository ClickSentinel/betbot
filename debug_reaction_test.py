...
async def test_debug_reaction_processing():
    ...
    with patch('cogs.betting.load_data', return_value=mock_data) as mock_load, \
         patch('cogs.betting.save_data') as mock_save, \
         patch('cogs.betting.schedule_live_message_update') as mock_schedule, \
         patch('cogs.betting.ensure_user') as mock_ensure:
        ...
        print(f"Processing reaction for user {payload.user_id}, emoji {str(payload.emoji)}")
        
        # Add debugging to see if the method is reached
        original_method = cog.on_raw_reaction_add
        
        async def debug_reaction_add(payload):
            print(f"→ on_raw_reaction_add called with user_id={payload.user_id}, bot_id={bot.user.id}")
