...
from config import (
    DATA_FILE, STARTING_BALANCE, ENABLE_BET_TIMER_DEFAULT,
    REACTION_BET_AMOUNTS, C1_EMOJIS, C2_EMOJIS,
...
def load_data() -> Data:
    ...
    if "betting" not in data:
        ...
        if "open" not in data["betting"]: data["betting"]["open"] = False; modified = True
        if "locked" not in data["betting"]: data["betting"]["locked"] = False; modified = True
        if "bets" not in data["betting"]: data["betting"]["bets"] = {}; modified = True
        if "contestants" not in data["betting"]: data["betting"]["contestants"] = {}; modified = True
    ...
    for user_id in list(data["balances"].keys()):
        if user_id not in data["balances"]: # This condition is technically redundant if "balances" is initialized
